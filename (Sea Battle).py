...
class BattleshipGame:
    ...
    def handle_setup(self):
        ...
        if self.manual_placement:
            ...
            # Показываем только активную доску
    ...
    def get_next_ship_size(self):
        ...
        # Если текущий размер больше не доступен, ищем следующий доступный
            if self.ships_to_place[self.selected_ship_size] == 0:
                for size in [4, 3, 2, 1]:
    ...
    # Если текущий размер закончился, ищем следующий доступный
        for size in [4, 3, 2, 1]:
            if self.ships_to_place[size] > 0:
                return size
        return None
    ...
    def handle_shot(self, row, col):
        ...
        if target_board[row][col] == 1:  # Попадание
    ...
    def computer_turn(self):
        pygame.time.wait(1000)  # Добавляем задержку перед ходом компьютера
        ...
        while True:
            ...
            if self.board1[row][col] not in [2, 3]:
                ...
                if hit and not self.winner:
                    pygame.time.wait(500)  # Задержка перед следующим ходом при попадании
                    continue  # Продолжаем ходить при попадании
                break  # Выходим после промаха
        ...
        while True:
            ...
            if self.menu_state == "MAIN_MENU":
                ...
                if not self.player1_name:
                    ...
                    # Отображение информации о текущем корабле
                    if self.selected_ship_size:
                        ship_info = f"Размещение корабля размером {self.selected_ship_size} для {self.player1_name if self.current_player == 1 else self.player2_name}"
                        ship_info_surface = self.font.render(ship_info, True, WHITE)
                        self.screen.blit(ship_info_surface, (10, 10))
                ...
                # Отображение имён игроков и текущего хода
